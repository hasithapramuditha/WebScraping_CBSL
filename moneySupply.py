...
def render_money_supply_page():
    st.header("Money Supply and Market Operations")
    
    # Load the data
    file_path = DOWNLOAD_DIR / "open_market_operations_clean_direct.csv"
    ...
    # Display latest data
    st.subheader("Latest Money Supply Data")
    latest_date = df['Date'].max()
    latest_data = df[df['Date'] == latest_date]
    
    # Create a table for the latest data
    st.write(f"Data as of: {latest_date.strftime('%Y-%m-%d')}")
    st.dataframe(latest_data[['Indicator', 'Value']].set_index('Indicator'))
    ...
    selected_indicators = st.multiselect(
        ...
        options=indicators,
        default=indicators[0] if len(indicators) > 0 else None
    )
    ...
    if selected_indicators:
        ...
        fig = px.line(
            ...
            y='Value',
            color='Indicator',
            title='Money Supply Indicators Over Time'
        )
